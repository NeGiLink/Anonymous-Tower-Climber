        ...
        private void RidingObjectUpdate()
        {
            ...
            foreach (Rigidbody2D ridingObject in mRidingObjects)
            {
                if (ridingObject != null)
                {
                    ...
                    ridingObject.position += mRigidbody2D.linearVelocity * Time.fixedDeltaTime;
                }
            }
        }
        ...
        private void OnCollisionEnter2D(Collision2D collision)
        {
            if (collision.gameObject.CompareTag("Player"))
            {
                ...
                if (playerRigidbody != null && !mRidingObjects.Contains(playerRigidbody))
                {
                    ...
                    playerRigidbody.transform.SetParent(transform); // 足場の子オブジェクトにする
                }
            }
        }
        ...
        private void OnCollisionExit2D(Collision2D collision)
        {
            if (collision.gameObject.CompareTag("Player"))
            {
                ...
                if (playerRigidbody != null && mRidingObjects.Contains(playerRigidbody))
                {
                    ...
                    playerRigidbody.transform.SetParent(null); // 足場の子オブジェクトから外す
                }
            }
        }
    }
